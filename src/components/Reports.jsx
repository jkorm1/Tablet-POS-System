...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const calculateData = () => {
        const totalSales = cards.reduce((sum, card) => sum + parseFloat(card.amount.slice(1)), 0);
        const transactions = cards.length;
        const avgTransaction = (totalSales / transactions).toFixed(2);
        const topProducts = cards.map(card => card.items[0].name);
        const salesData = {
          labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
          datasets: [
            {
              label: 'Sales ($)',
              data: Array(12).fill(0).map((_, index) => cards.reduce((sum, card) => sum + (index % 3 === 0 ? parseFloat(card.amount.slice(1)) : 0), 0)),
      ...
      const response = await new Promise((resolve) =>
        setTimeout(() => resolve(calculateData()), 1000)
      );
